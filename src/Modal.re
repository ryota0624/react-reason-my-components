...
type modalSize = Large 
  | Small
  | Medium

let modalSizeToAttr = size => switch (size) {
| Large => "modal-lg"
| Small => "modal-sm"
| Medium => ""
}

let modalOpenStyle = ReactDOMRe.Style.make(
    ~display = "block",
    ~visibility = "visible",
    ~transition = "all 0.5s",
    ~opacity = "1"
...
let modalHiddenStyle = ReactDOMRe.Style.make(
  ~display = "block",
  ~visibility = "hidden",
  ~transition = "all 0.5s",
  ~top= "-100vh",
...
let make = (~hidden, ~size, ~onClickContentOutside=?, ~header=?, ~footer=?, ~body=?, _children) => {
  ...component,
  render: _ => {
      <div
        ...
        <div className=("modal-dialog " ++ (modalSizeToAttr (size))) 
          onClick=(event => {
