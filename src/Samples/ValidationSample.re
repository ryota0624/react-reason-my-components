...
type validationFailure = TextIsOverMaxLength | TextIsEmpty;
...
let validateAge = (getAge) =>  Validation.fromErrors((record) => 
if (getAge(record) >= 0) {
  []
} else [InvalidAge]
);

let validateName = (getName) =>  Validation.fromErrors((record) => {
  if ((getName(record)) == "") {
    [InvalidName]
  } else []
});
...
let () = {
  ...
  let _ = Validation.run(Validation.all([
    ifEmptyString((v) => v, [TextIsEmpty]),
    validateText
  ]), "");
